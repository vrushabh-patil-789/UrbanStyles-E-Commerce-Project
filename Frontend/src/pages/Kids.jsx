  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setProducts(data.filter(p => p.category === "kids"));
      } catch (error) {
      ...
      <section ref={ref} className={`px-6 md:px-20 py-14 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {loading ? (
              <p className="col-span-full text-center py-10 dark:text-white">Loading products...</p>
            ) : products.length === 0 ? (
              <p className="col-span-full text-center py-10 dark:text-white">No products found.</p>
            ) : (
